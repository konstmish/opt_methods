...
class Trainer:
    ...
    """
    def __init__(self, oracle, t_max=np.inf, it_max=np.inf, output_size=500, tolerance=0, verbose=False):
    ...
    def check_convergence(self):
        ...
        tolerance_met = la.norm(self.w - self.w_old) <= self.tolerance
    ...
    def compute_loss_of_iterates(self):
        if self.losses is None:
            self.losses = np.array([self.oracle.loss_func(x) for x in self.xs])
        elif self.verbose:
    ...
    def plot_distances(self, label='', marker='', x_opt=None, markevery=None):
        if x_opt is None:
            if self.losses is None:
                ...
                x_star = self.xs[i_min]
        if markevery is None:
            markevery = max(1, len(self.xs) // 20)
        plt.plot(self.its, la.norm(self.xs-x_star, axis=1), label=label, marker=marker, markevery=markevery)
